<div class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-1 left-0 z-50 justify-center items-center text-left w-full md:inset-0 h-[calc(100%-1rem)] max-h-full"
    id="edit-alternative-{{ $alternative->id }}" aria-hidden="true">
    <div
        ...
        <div
            ...
            <form id="edit-form-{{ $alternative->id }}" action="{{ route('alternatives.update', $alternative->id) }}"
                ...
                <div
                    ...
                    <h5 class="mb-0 leading-normal" id="ModalLabel">Ubah data alternative</h5>

                    <button type="button" data-modal-toggle="edit-alternative-{{ $alternative->id }}"
                        class="fa fa-close w-4 h-4 ml-auto box-content p-2 text-black border-0 rounded-1.5 opacity-50 cursor-pointer -m-2 "
                        data-dismiss="edit-alternative-{{ $alternative->id }}"></button>
                </div>
                <div class="relative flex-auto p-4">
                    <input type="hidden" name="id" value="{{$alternative->id}}">
                    <p>Kode Alternative</p>
                    <p class=" text-xl font-semibold mb-4">A{{ $alternative->id }}</p>
                    <p>Nama Alternative</p>
                    <input dropzone name="name" id="name" type="text" value="{{$alternative->name}}"
                ...
                <div
                    ...
                    <button type="submit" data-toggle="modal" data-target="#import"
                        class="inline-block px-8 py-2 m-1 mb-4 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-gradient-to-tl from-purple-700 to-pink-500 shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85">Ubah Data</button>
                </div>
            </form>
        </div>
    </div>
